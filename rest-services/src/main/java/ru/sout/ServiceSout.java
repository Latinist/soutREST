...
//-----------------------------------------Measure------------------------------------------------
    ...
    private void setDaoValues(List<FactorsValue> data, String token) throws Exception {
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
        try {
            //String res = sp.setValues(data);
            String par = sp.setValuesT1(data,token);
            sp.doRenum(token);
            logger.info("------setValue Ok ok------");
...
//            };
    ...
    @Path("/recalc/{id}")
    @Produces(MimeType.TEXT)
    public String doRecalc(@PathParam("id") String token){
        return daoDoRecalc(token);
    }

    private String daoDoRecalc(String token) {
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));

//        List<Errors> errorsList = null;
//        errorsList = daoDoFuzzyRecalc(token);

        String res = sp.doRecalc(token);
        if (!res.equals("")){
            res = sp.doRecalc(token); // вызываем второй раз на случай необходимости активации пакета
            if (!res.equals("")) {
                logger.info("Ошибка расчета рабочего места");
                res = "Ошибка расчета рабочего места:" + res;
            }
        } else {
            res = "Ok";
        };
    ...
    @Path("/fuzzyrecalc/{id}")
    @Produces(MimeType.JSON8)
    public List<Errors> doFuzzyRecalc(@PathParam("id") String token){
    return daoDoFuzzyRecalc(token);
}
    private List<Errors> daoDoFuzzyRecalc(String token) {
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
        return sp.doFuzzyRecalc(token);
    }
...
//-----------------------------------------------------------------------------------------
    ...
    public String updateWorkplace(@PathParam("id") String token, List<WorkPlace> data) {
        ...
        System.out.println("---------------------------------------");
        try {
            String err = setDaoWorkplace(data, token);
            if (err == null) {
                return "Ok";
            } else {
                return "Ошибка: "+ err;
    ...
    private String setDaoWorkplace(List<WorkPlace> data,String token) {
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
...
//            }
//        }
        return sp.setWorkplace(data, token);
    }
//-----------------------------------------html------------------------------------------------
@GET
...
@Path("/kart2050/{id}")
@Produces(MimeType.JSON8)
public List<GetStructure2050> getkart2050(@PathParam("id") String id){
    return getDaoKart2050(id);
}

    private List<GetStructure2050> getDaoKart2050(String token){
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
...
@Consumes(MimeType.JSON)
@Produces(MimeType.TEXT)
public String updateKart2050(@PathParam("id") String token, List<GetStructure2050> data) {
    System.out.println("---------------------------------------");
    ...
    private void setDaoKarts2050(List<GetStructure2050> data) {
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
...
@GET
@Path("/version/mobile/{id}")
@Produces(MimeType.TEXT)
    public String getVersionMobile(@PathParam("id") String token){return getDaoVersionMobile(token);
    }

    private String getDaoVersionMobile(String token){
        String timeStamp;
        String path=System.getProperty("user.dir");
        logger.info("-----------Корневой путь РЭСТ:"+path);
//        String mobilePuth = path.substring(0,path.lastIndexOf("bin")).concat("webapps\\DISTRIBUT\\MOBILE\\sout-release.apk");
        String mobilePuth = null;
        if (path.contains("bin")) {
            mobilePuth = path.substring(0, path.lastIndexOf("bin")).concat("webapps\\DISTRIBUT\\MOBILE");
        }else{
            mobilePuth = path.concat("\\webapps\\DISTRIBUT\\MOBILE");
        }
        File dir = new File(mobilePuth);
        if (dir.isDirectory()){
            String[] files = dir.list(new FilenameFilter() {
                @Override
                public boolean accept(File dir, String name) {
                    return name.endsWith(".apk") && name.startsWith("sout-");
                }
            });
            int max = 0;
            String name = "";
            for (String filename: files){
                String s = filename.substring(5,filename.indexOf("-",5));
                try {
...
//        return timeStamp;
    ...
    @Path("/ilive")
    @Produces(MimeType.TEXT)
    public String getILive(){return "True";
    }

    //-----------------------------------------Версия десктопа------------------------------------------------
    @GET
    @Path("/version/desktop/{id}")
    @Produces(MimeType.TEXT)
    public String getVersionDesktop(@PathParam("id") String token){return getDaoVersionDesktop(token);
    }

    private String getDaoVersionDesktop(String token){
        String timeStamp;
        String path=System.getProperty("user.dir");
        String mobilePuth = null;
        if (path.contains("bin")) {
            mobilePuth = path.substring(0, path.lastIndexOf("bin")).concat("webapps\\DISTRIBUT\\DESKTOP");
        }else{
            mobilePuth = path.concat("\\webapps\\DISTRIBUT\\DESKTOP");
        }
        File dir = new File(mobilePuth);
        if (dir.isDirectory()){
            String[] files = dir.list(new FilenameFilter() {
                @Override
                public boolean accept(File dir, String name) {
                    //return name.endsWith(".exe") && name.startsWith("SOUT-");
                    return name.startsWith("SOUT-");
                }
            });
            int max = 0;
            String name = "";
    ...
    @Path("/listjobs/")
    @Produces(MimeType.JSON8)
    @Consumes(MimeType.JSON8)
    public List<LispJobs> getLispJobs(Map<String,String> data){return getDaoLispJobs(data);}
    private List<LispJobs> getDaoLispJobs(Map<String,String> data) {
//        String decodeLogin = RLDecoder.decode( login, "UTF-8" );
        String login = data.get("login");
        String password = data.get("password");
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
        return sp.getLispJobs(login, password);
    }
    ...
    @Produces(MimeType.TEXT)
    @Consumes(MimeType.JSON8)
    public String getTestUser(Map<String,String> data){return getDaoTestUser(data);}
    private String getDaoTestUser(Map<String,String> data) {
        String login = data.get("login");
        String password = data.get("password");
        ServiceSprav sp =  ((ServiceSprav) SpringApplicationContext.getBean("serviceSprav"));
        return sp.getTestUser(login, password);
    }

//    String mobilePuth = path.substring(0,path.lastIndexOf("bin")).concat("webapps\\DISTRIBUT\\DESKTOP\\sout-release.apk");
//    File file = new File(mobilePuth);
//    if (file.exists()){
//        Long time = file.lastModified();
////            Date date = new Date(file.lastModified());
